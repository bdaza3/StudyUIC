...
export async function getCourses(): Promise<Course[]> {
  const { data, error } = await getSupabaseClient()
    .from("courses")
    .select("id,department,course_number,course_code,title,description")
    .eq("active", true)
    .order("course_code");
...
export async function getStudySessionsForSpot(
  spotId: string,
  courseId?: string | null,
): Promise<StudySession[]> {
  const { data, error } = await getSupabaseClient().rpc(
    "get_sessions_for_spot",
    { p_spot_id: spotId, p_course_id: courseId ?? null },
  );
  if (error) throw new Error("Unable to load study sessions.");
  return data as StudySession[];
}

export async function getActiveStudySessions(
  courseId?: string | null,
): Promise<StudySession[]> {
  const { data, error } = await getSupabaseClient().rpc(
    "get_active_study_sessions",
...
export async function getSessionMembers(
  ...
  const { data, error } = await getSupabaseClient().rpc("get_session_members", {
